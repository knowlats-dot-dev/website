...
pnpm build      # Build for production (also runs postbuild search index script)
...
Full-text search uses [lunr](https://lunrjs.com/). The index at `public/search-index.json` is generated by `scripts/search/prepare-index.js` which runs automatically after `pnpm build` (postbuild hook). It reads frontmatter + body from every `.md` file in `src/content/blog/`.
