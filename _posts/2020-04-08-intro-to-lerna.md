...
![Photo by [Jack B](https://unsplash.com/@nervum) on [Unsplash](https://unsplash.com)](https://images.unsplash.com/photo-1529981188441-8a2e6fe30103?ixlib=rb-1.2.1&ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&auto=format&fit=crop&w=750&q=80)*Photo by [Jack B](https://unsplash.com/@nervum) on [Unsplash](https://unsplash.com)*


Project ของ NPM (Node Package Manager) ไม่ว่าจะเป็น Project ที่สร้างขึ้นมาด้วยคำสั่ง npm init Project ที่เป็น React, Vue, หรืออะไรที่เป็น JavaScript ที่มีไฟล์ package.jsonจะถือว่าเป็น 1 Project
...
ที่โฟลเดอร์ Service สร้าง Service ตัวแรกชื่อ backend-service
...
เพิ่มไฟล์ index.jsให้กับ backend-service

{% gist 908ccb49daf45de5585ed2aa0432fb0d %}

สำหรับ web-serve-service

ในที่นี้จะใช้ ejs เป็น Web Template Engine เพิ่มเติม ให้ติดตั้งลงไป
...
หลังจากใช้คำสั่งนี้ @lerna-test/logger ควรอยู่ในรายชื่อ dependenciesในไฟล์ package.json ของทั้งสอง Service
...
เราสามารถพิมพ์คำสั่ง lerna add แล้วตามด้วยชื่อ package ได้เช่น
...
จากคำสั่งนี้แปลความหมายได้ว่า ให้ลง express ในทุก Project ที่ชื่อมี @lerna-testตามด้วยตัวอักษรใด ๆ ข้างหน้า-service
...
ข้อแตกต่างคำสั่งที่ไม่มี --parallel และคำสั่งที่มี --parallelจะเห็นได้จาก Console ดังตัวอย่าง

![เปรียบเทียบผลลัพธ์ของคำสั่งที่ไม่มี — parallel และมี — parallel](/assets/image/post/lerna02.png)*เปรียบเทียบผลลัพธ์ของคำสั่งที่ไม่มี — parallel และมี — parallel*
...
> คำสั่ง lerna runทำงานเหมือน lerna exec แต่สามารถรันได้เฉพาะ NPM Script เท่านั้น
...
สำหรับการลบ Package เราจะใช้คำสั่ง npm uninstallไม่ได้เพราะว่าคำสั่งนี้มีการสแกนไฟล์ package.json เพื่อเช็คซอร์สโค้ดแต่ละ Package ด้วย ก่อนที่จะลบ Package ที่เลือกออกไป และมันจะทำไม่สำเร็จเพราะหา Package บางตัวไม่เจอเช่นเดียวกันตอนใช้คำสั่ง npm install
...
ที่ผ่านมา เรายุ่งกับไฟล์ package.json ที่เป็นของแต่ละ Subproject เท่านั้น ในหัวข้อนี้ จะมาพูดถึงไฟล์ Package.json ที่เป็นของ Project หลัก (อยู่ใน Root ของ Project) สักเล็กน้อย

ไฟล์ package.json นี้เราสามารถที่จะจัดการตามปกติได้เลย เช่น การติดตั้ง Package โดยใช้ npm install(Package ที่ติดตั้งตรงนี้จะใช้งานกับทุก Subproject ได้) การลบ Package โดยใช้ npm uninstall หรือการเพิ่ม NPM Script ใด ๆ หรือแม้แต่การเพิ่มให้รันด้วย npm แทน lernaก็ทำได้เช่นกัน ดังตัวอย่าง
...
![Photo by [Les Triconautes](https://unsplash.com/@triconautes) on [Unsplash](https://unsplash.com)](https://images.unsplash.com/photo-1553585988-4319f2e6f542?ixlib=rb-1.2.1&ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&auto=format&fit=crop&w=889&q=80)*Photo by [Les Triconautes](https://unsplash.com/@triconautes) on [Unsplash](https://unsplash.com)*
...
นอกจากช่วยประหยัดพื้นที่ฮาร์ดดิสก์แล้ว การแชร์ node_modulesยังช่วยอำนวยสะดวกให้หลายอย่าง เราสามารถจัดการ Dependency ทั้งการเพิ่ม Package และการลบ Package ได้ด้วยคำสั่งของ Yarn คำสั่งเดียว
...
เสร็จสิ้น เราได้เปิดใช้ Yarn Workspace ใน Project แล้ว
...
* **การติดตั้ง Common Package **— หากเราจะติดตั้ง Package ที่ Root ของ Project เราต้องใช้คำสั่ง yarn add <package-name> -Wเพื่อให้ Yarn ยอมรับการติดตั้ง Package ที่ Root ของ Project (-W มีชื่อ Option เต็ม ๆ คือ--ignore-workspace-root-check)
